...
function createMaskGeoJSON(center) {
  ...
  return { type: 'Feature', geometry: { type: 'Polygon', coordinates: [outerRing, innerRing] } };
}
...
function createLockPoints(center, radiusMeters, count = 16) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    points.push({
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [lng + deltaLng, lat + deltaLat] },
      properties: { icon: '🔒' }
    });
  }
...
function MetricCard({ label, value, unit, trend, trendLabel, barPct, barColor, icon }) {
  const isPositive = trend >= 0;
  return (
    <div className="mv-metric-card">
      <p className="mv-metric-label">{icon} {label}</p>
      <div className="mv-metric-row">
        <span className="mv-metric-value">{value}<span className="mv-metric-unit">{unit}</span></span>
        {trend !== undefined && (
          <span className={`mv-metric-trend ${isPositive ? 'trend-up' : 'trend-down'}`}>
      ...
      <div className="mv-bar-track">
        <div className="mv-bar-fill" style={{ width: `${barPct}%`, background: barColor }} />
      </div>
    </div>
  );
}
...
function PollutionChart({ pollution }) {
  ...
  return (
    <svg className="mv-pollution-svg" viewBox={`0 0 ${totalW} ${maxH + 28}`} width="100%" height={maxH + 28}>
      {items.map((item, i) => {
        const h = Math.max(4, Math.min(maxH, (item.value / item.max) * maxH));
        const x = i * (barW + gap);
        const pct = (item.value / item.max) * 100;
        const fill = pct < 30 ? '#22c55e' : pct < 60 ? '#eab308' : pct < 80 ? '#f97316' : '#ef4444';
        return (
          <g key={item.label}>
            <rect x={x} y={maxH - h} width={barW} height={h} fill={fill} rx={5} opacity={0.85} />
            <text x={x + barW / 2} y={maxH - h - 5} textAnchor="middle" fontSize="9" fontWeight="700" fill="#475569">
              {item.value.toFixed(1)}
            </text>
            <text x={x + barW / 2} y={maxH + 14} textAnchor="middle" fontSize="10" fontWeight="600" fill="#64748b">
              {item.label}
            </text>
          </g>
        );
...
function ToggleBtn({ icon, label, active, onToggle }) {
  return (
    <button className={`mv-toggle-btn ${active ? 'mv-on' : 'mv-off'}`} onClick={onToggle}>
      <span className="mv-toggle-icon">{icon}</span>
      <span className="mv-toggle-label">{label}</span>
      <span className={`mv-toggle-pill ${active ? 'mv-pill-on' : 'mv-pill-off'}`}>
        {active ? 'ON' : 'OFF'}
      </span>
    </button>
  );
...
function MapView() {
  ...
  const toggleTraffic   = useCallback(() => { const v = !trafficOn;   setTrafficOn(v);   dispatch('traffic',   v); }, [trafficOn]);
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      map.current.addLayer({
        ...
        type: 'symbol',
        source: 'lock-icons',
        layout: { 'text-field': ['get', 'icon'], 'text-size': 26, 'text-anchor': 'center' }
      });
    ...
    <div className="mv-dashboard">
      ...
      <nav className="mv-navbar">
        ...
        <div className="mv-nav-right">
          <span className={`mv-status-chip ${connected ? 'mv-online' : 'mv-offline'}`}>
            <span className={`mv-status-dot ${connected ? 'mv-dot-on' : 'mv-dot-off'}`} />
      ...
      <div className="mv-status-bar">
        <div className="mv-status-left">
          ...
          <span className="mv-status-sep">|</span>
          <span className="mv-status-info">{formattedDate}, {formattedTime}</span>
          <span className="mv-status-sep">|</span>
          <span className="mv-status-info">Status: <strong className="mv-status-active">ACTIVE</strong></span>
        </div>
      </div>
      ...
      <div className="mv-body">
        ...
        {/* ── Left Sidebar ───────────────────────────────── */}
        <aside className={`mv-left-panel ${isLeftOpen ? 'mv-open' : 'mv-closed'}`}>
          <h3 className="mv-section-title">CITY METRICS</h3>

          <MetricCard
            icon="🌡️" label="TEMPERATURE"
            value={weather.main.temp.toFixed(1)} unit="°C"
            trend={feelsLikeDelta} trendLabel="°C"
            barPct={normalizeRange(weather.main.temp, WEATHER_SCALES.temp.min, WEATHER_SCALES.temp.max)}
            barColor="linear-gradient(90deg,#f97316,#ef4444)"
          />

          <MetricCard
            icon="💨" label="AIR QUALITY"
            value={pollution.aqi} unit=" AQI"
          ...
          <MetricCard
            icon="💧" label="HUMIDITY"
            value={weather.main.humidity.toFixed(0)} unit="%"
            barPct={normalizeMax(weather.main.humidity, WEATHER_SCALES.humidity.max)}
            barColor="linear-gradient(90deg,#38bdf8,#0ea5e9)"
          />

          <MetricCard
            icon="🌀" label="WIND SPEED"
            value={weather.wind.speed.toFixed(1)} unit=" m/s"
          ...
          <MetricCard
            ...
            value={weather.clouds.toFixed(0)} unit="%"
          ...
          <div className="mv-toggle-section">
            ...
            <ToggleBtn icon="🚗" label="Traffic"     active={trafficOn}   onToggle={toggleTraffic}   />
          </div>
        </aside>
        ...
        {/* ── Right Sidebar ──────────────────────────────── */}
        <aside className={`mv-right-panel ${isRightOpen ? 'mv-open' : 'mv-closed'}`}>
          ...
          <div className="mv-analytics-card">
            <div className="mv-card-header">
              ...
              {weatherIconUrl && <img className="mv-weather-icon" src={weatherIconUrl} alt={weather.condition.main} />}
            </div>
            <p className="mv-weather-desc">{weather.condition.main} · {weather.condition.description}</p>
            <div className="mv-weather-grid">
              <div className="mv-wg-cell">
                <span className="mv-wg-val mv-temp-accent">{weather.main.temp.toFixed(1)}°C</span>
                <span className="mv-wg-lbl">TEMP</span>
              </div>
              <div className="mv-wg-cell">
                <span className="mv-wg-val">{weather.main.humidity.toFixed(0)}%</span>
                <span className="mv-wg-lbl">HUMIDITY</span>
              </div>
              <div className="mv-wg-cell">
                <span className="mv-wg-val">{weather.wind.speed.toFixed(1)} <small>m/s</small></span>
                <span className="mv-wg-lbl">WIND</span>
              </div>
              <div className="mv-wg-cell">
                <span className="mv-wg-val">{weather.main.pressure.toFixed(0)} <small>hPa</small></span>
          ...
          <div className="mv-analytics-card mv-alert-card">
            ...
            <div className="mv-alert-list">
              {events.length === 0 && <p className="mv-no-events">No active events</p>}
              {events.slice(0, 6).map((evt, i) => (
                <div key={evt._id || i} className="mv-alert-item">
                  <div className="mv-alert-dot-wrap">
                    <span className={`mv-alert-dot ${evt.status === 'active' ? 'mv-dot-alert' : 'mv-dot-info'}`} />
                  </div>
                  <div className="mv-alert-body">
                    <p className="mv-alert-name">{evt.name}</p>
                    <p className="mv-alert-time">
                      {evt.start_at ? new Date(evt.start_at).toLocaleString('en-IN', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'N/A'}
                    </p>
                  </div>
                  <span className={`mv-alert-status ${evt.status === 'active' ? 'mv-status-warn' : 'mv-status-ok'}`}>
                    {evt.status || 'info'}
                  </span>
                </div>
              ))}
